...
def _():
    ...
    return (datetime, logger, log_stream, current_time_str, np, pl, mo)
...
def _(mo):
    setup_form = (
        ...
        .batch(
            ...
            reward_range=mo.ui.text(label="", value="10,50", placeholder="min,max"),
            cost_range=mo.ui.text(label="", value="5,20", placeholder="min,max"),
...
def _(setup_form, np, pl, mo, logger, current_time_str):
    ...
    def initialize_experiment():
        ...
        logger.info(
            f"Experiment initialized: {n_trials} trials at {current_time_str()}"
        )
        return df
    ...
    return (
        ...
        get_choice_log,
        set_choice_log,
        get_experiment_started,
        set_experiment_started,
    )
...
def _(
    ...
    progress_pct = (
        int((current_trial - 1) / total_trials * 100) if show_progress.value else None
    )
...
def _(
    ...
    trial_start_time = (
        mo._runtime.get_pyodide_mounted_element_time()
        if hasattr(mo._runtime, "get_pyodide_mounted_element_time")
        else None
    )
    ...
    def make_choice(choice_label):
        ...
        actual_choice = (
            "Option 1"
            if (choice_label == "Option 1" and position_mode == "Constant")
            or (choice_label == opt1_label)
            else "Option 2"
        )
        ...
        updated_log = (
            pl.concat([choice_log, new_entry]) if choice_log is not None else new_entry
        )
        set_choice_log(updated_log)
        set_current_trial(current_trial + 1)
        set_trial_start(datetime.datetime.now())
        logger.info(
            f"Trial {trial_row['trial']} | Choice: {actual_choice} | RT: {rt_ms}ms"
        )
    ...
    if layout_mode == "Left-Right":
            ...
            [
                mo.vstack(
                    [
                        ...
                        mo.md(opt1_text),
                        mo.ui.button(
                            label="Select", on_click=lambda _: make_choice(opt1_label)
                        ),
                    ]
                ),
                mo.vstack(
                    [
                        ...
                        mo.md(opt2_text),
                        mo.ui.button(
                            label="Select", on_click=lambda _: make_choice(opt2_label)
                        ),
                    ]
                ),
            ]
        ...
        mo.vstack(
            [
                ...
                mo.ui.button(
                    label=f"Select {opt1_label}",
                    on_click=lambda _: make_choice(opt1_label),
                ),
                mo.md(f"**{opt2_label}**: {opt2_text}"),
                mo.ui.button(
                    label=f"Select {opt2_label}",
                    on_click=lambda _: make_choice(opt2_label),
                ),
            ]
        )
